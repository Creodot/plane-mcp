...
export const CreatePlaneIssuePayloadSchema = z.object({
  ...
  priority: z.enum(["urgent", "high", "medium", "low", "none"]).nullish().describe("The priority of the issue."),
  state: z.string().uuid().nullish().describe("The ID of the state to assign the issue to."),
  assignees: z.array(z.string().uuid()).optional().describe("An array of user IDs to assign to the issue."),
  labels: z.array(z.string().uuid()).optional().describe("An array of label IDs to assign to the issue."),
...
export const UpdatePlaneIssuePayloadSchema = z.object({
  ...
  description: z.string().nullish().optional().describe("The new description for the issue (plain text)."),
  description_html: z.string().nullish().optional().describe("The new description for the issue (HTML)."),
  priority: z.enum(["urgent", "high", "medium", "low", "none"]).nullish().optional().describe("The new priority for the issue."),
