...
import type { CreatePlaneIssuePayload, PlaneIssue, UpdatePlaneIssuePayload } from '../src/types/plane-types.js';
...
describe('IssueService', () => {
  ...
  it('getIssue should call planeClient.request with correct parameters', async () => {
    const mockIssue: PlaneIssue = {
      id: mockIssueId, name: 'Test Issue', description: 'Desc', description_html: '<p>Desc</p>',
      priority: 'medium', state: 'state-1', state_detail: null, project: mockProjectId,
      workspace: mockWorkspaceSlug, created_at: new Date().toISOString(), updated_at: new Date().toISOString(),
      created_by: 'user-1', updated_by: 'user-1', assignees: [], assignee_details: [],
      labels: [], label_details: [], parent: null
    };
    ...
    expect(planeClient.request).toHaveBeenCalledWith(
      ...
      { method: 'GET' },
      { workspace_slug: mockWorkspaceSlug, project_id: mockProjectId, issue_id: mockIssueId }
    );
    expect(result).toEqual(mockIssue);
  });

  it('createIssue should call planeClient.request with correct parameters', async () => {
    const payload: CreatePlaneIssuePayload = { name: 'New Issue', project: mockProjectId };
    const mockCreatedIssue: PlaneIssue = { 
       id: 'new-issue-789', name: 'New Issue', description: null, description_html: null,
      priority: null, state: 'state-1', state_detail: null, project: mockProjectId,
      workspace: mockWorkspaceSlug, created_at: new Date().toISOString(), updated_at: new Date().toISOString(),
      created_by: 'user-1', updated_by: 'user-1', assignees: [], assignee_details: [],
      labels: [], label_details: [], parent: null
    };
    ...
    const payload: Omit<CreatePlaneIssuePayload, 'project'> = { name: 'New Issue No Project' }; // Project missing
    const expectedPayloadWithProject: CreatePlaneIssuePayload = { ...payload, project: mockProjectId }; 
    const mockCreatedIssue: PlaneIssue = { 
       id: 'new-issue-789', name: 'New Issue No Project', description: null, description_html: null,
      priority: null, state: 'state-1', state_detail: null, project: mockProjectId,
      workspace: mockWorkspaceSlug, created_at: new Date().toISOString(), updated_at: new Date().toISOString(),
      created_by: 'user-1', updated_by: 'user-1', assignees: [], assignee_details: [],
      labels: [], label_details: [], parent: null
    ...
    await issueService.createIssue(mockWorkspaceSlug, mockProjectId, payload as CreatePlaneIssuePayload);
  ...
  it('updateIssue should call planeClient.request with correct parameters', async () => {
    const payload: UpdatePlaneIssuePayload = { name: 'Updated Issue Name' };
     const mockUpdatedIssue: PlaneIssue = { 
       id: mockIssueId, name: 'Updated Issue Name', description: 'Desc', description_html: '<p>Desc</p>',
      priority: 'medium', state: 'state-1', state_detail: null, project: mockProjectId,
      workspace: mockWorkspaceSlug, created_at: new Date().toISOString(), updated_at: new Date().toISOString(),
      created_by: 'user-1', updated_by: 'user-1', assignees: [], assignee_details: [],
      labels: [], label_details: [], parent: null
    ...
    const result = await issueService.updateIssue(mockWorkspaceSlug, mockProjectId, mockIssueId, payload);
